...
const project = {
  ...
  description:
    "An RC522 RFID reader checks cards/fobs against an allowlist stored in Arduino memory. Granted = green LED + relay clicks. Denied = red LED + buzzer. Every attempt is logged over Serial with a timestamp — your first taste of access control logs, replay attacks, and physical security.",
  tinkercad:
    "Prototype in TinkerCAD Circuits using Arduino Uno + simulated components, then transfer to physical build.",
  github:
    "Code goes on GitHub: Arduino .ino sketch + a README documenting the circuit, wiring diagram, and security notes.",
  cyber:
    "Ties to: Authentication (UID as credential), Access Logs (serial monitor), Replay Attacks (cloned RFID cards), Physical Security vs Digital Security.",
};
...
const curriculum = [
  {
    ...
    project_step:
      "TinkerCAD: Build LED + button circuit. Button press logs 'ACCESS ATTEMPT' to Serial.",
  ...
  },
  {
    ...
    project_step:
      "TinkerCAD: Simulate RFID scan (TinkerCAD has limited RFID sim — use button as stand-in, test logic). Physical: Read real card UIDs.",
  ...
  },
  {
    ...
    project_step:
      "TinkerCAD: Wire relay module — trigger relay on button press. Add LED indicators (green/red).",
  ...
  },
  {
    ...
    project_step:
      "Full working prototype in TinkerCAD (partial simulation) + physical breadboard build.",
    resources: [],
  },
  {
    week: "Week 7",
  ...
  },
  {
    ...
    project_step:
      "Research only — no build needed. Great for your README's 'Security Notes' section.",
...
function App() {
  const [weeks, setWeeks] = useState(
    curriculum.map((week) => ({
      ...week,
      topics: week.topics.map((topic) => ({ ...topic })),
    }))
  );
  const [activeWeek, setActiveWeek] = useState(0);
  const [tab, setTab] = useState("curriculum");

  const toggleTopic = (weekIndex, topicIndex) => {
    setWeeks((prevWeeks) =>
      prevWeeks.map((week, currentWeekIndex) =>
        currentWeekIndex !== weekIndex
          ? week
          : {
              ...week,
              topics: week.topics.map((topic, currentTopicIndex) =>
                currentTopicIndex !== topicIndex ? topic : { ...topic, done: !topic.done }
              ),
            }
      )
    );
  ...
  const doneTopics = weeks.reduce(
    (sum, week) => sum + week.topics.filter((topic) => topic.done).length,
    0
  );
      ...
      >
        ...
        >
          ...
          >
            ...
            <span style={{ fontSize: 13, color: "#00ff9d", fontWeight: 600 }}>
              {doneTopics}/{totalTopics} topics · {pct}%
            </span>
          </div>
        ...
        >
          {["curriculum", "components", "project"].map((currentTab) => (
            <button
              key={currentTab}
              className="tab-btn"
              ...
              style={{
                ...
                border: "none",
                borderBottom:
                  tab === currentTab ? "2px solid #00ff9d" : "2px solid transparent",
                color: tab === currentTab ? "#00ff9d" : "#555",
                cursor: "pointer",
                fontFamily: "inherit",
                fontSize: 12,
                letterSpacing: 2,
          ...
          >
            <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              {weeks.map((week, weekIndex) => {
                const doneCount = week.topics.filter((topic) => topic.done).length;
                ...
                return (
                  <button
                    ...
                    style={{
                      padding: "10px 14px",
                      background: activeWeek === weekIndex ? `${week.color}15` : "transparent",
                      border: `1px solid ${activeWeek === weekIndex ? week.color : "#1e2128"}`,
                      borderRadius: 6,
                      cursor: "pointer",
                      fontFamily: "inherit",
                      textAlign: "left",
                  ...
                  >
                    <div style={{ fontSize: 10, color: week.color, letterSpacing: 1, marginBottom: 2 }}>
                      {week.week}
                    </div>
                    <div
                      style={{
                        fontSize: 11,
                        color: activeWeek === weekIndex ? "#fff" : "#666",
                        lineHeight: 1.3,
                      }}
                    >
                      {week.title}
            ...
            <div
              ...
              style={{
                padding: "20px 24px",
                border: `1px solid ${activeWeekData.color}30`,
                borderRadius: 8,
                background: "#0d0f14",
              }}
            >
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 2, marginBottom: 24 }}>
                {activeWeekData.topics.map((topic, topicIndex) => (
                  <div
                    key={topic.name}
                    className="topic-row"
                    onClick={() => toggleTopic(activeWeek, topicIndex)}
                    style={{
                      display: "flex",
                  ...
                  >
                    <div
                      style={{
                        width: 18,
                        height: 18,
                        border: `1.5px solid ${topic.done ? activeWeekData.color : "#333"}`,
                        borderRadius: 3,
                        flexShrink: 0,
                        marginTop: 1,
                        background: topic.done ? activeWeekData.color : "transparent",
                        display: "flex",
                        alignItems: "center",
              ...
              <div
                style={{
                  padding: "14px 16px",
                  background: `${activeWeekData.color}0d`,
                  border: `1px solid ${activeWeekData.color}30`,
                  borderRadius: 6,
                  marginBottom: activeWeekData.resources.length ? 16 : 0,
                }}
              >
                <div style={{ fontSize: 10, color: activeWeekData.color, letterSpacing: 2, marginBottom: 6 }}>
                  ▸ TINKERCAD / BUILD STEP
                </div>
                <p style={{ fontSize: 12, color: "#aaa", lineHeight: 1.6 }}>
                  {activeWeekData.project_step}
                </p>
              </div>

              {activeWeekData.resources.length > 0 && (
                <div style={{ marginTop: 16 }}>
                  <div style={{ fontSize: 10, color: "#444", letterSpacing: 2, marginBottom: 8 }}>
                    RESOURCES
                  </div>
                  {activeWeekData.resources.map((resource) => (
                    <div
                      key={resource}
                      style={{
                        fontSize: 12,
        ...
        {tab === "components" && (
          <div className="fade-in">
            <p style={{ color: "#666", fontSize: 12, marginBottom: 20 }}>
              Total estimated cost: ~$25–30. Everything available on Amazon or AliExpress.
            </p>
            <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
              {components.map((component) => (
                ...
                >
                  <span style={{ fontSize: 13, color: "#ccc" }}>{component.name}</span>
                  <span style={{ fontSize: 12, color: "#555", textAlign: "center" }}>×{component.qty}</span>
                  <span style={{ fontSize: 12, color: "#00ff9d", textAlign: "right" }}>
                    {component.cost}
                  </span>
                </div>
              ))}
            ...
            >
              <div style={{ fontSize: 10, color: "#00cfff", letterSpacing: 2, marginBottom: 10 }}>
                ▸ TINKERCAD SIMULATION NOTES
              </div>
              <p style={{ fontSize: 12, color: "#777", lineHeight: 1.7 }}>
                TinkerCAD Circuits has Arduino Uno, LEDs, buzzers, buttons, relays, and resistors built
                in. The RC522 RFID module isn't natively simulated — use a push button as a stand-in to
                test the access logic and Serial output. Once logic is verified, replicate on physical
                hardware.
              </p>
            </div>
          </div>
            ...
            {[
              {
                ...
                color: "#00ff9d",
                content:
                  "Reads RFID card UIDs. Checks against hardcoded allowlist. Grants access (green LED + relay) or denies (red LED + buzzer). Logs every attempt to Serial with millis() timestamp and UID.",
              },
              {
